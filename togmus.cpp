...
static int y(int x) { return (x>0) ? x + vars : x - vars; }
static int a(int x) { return x + 2*vars; }
static int b(int x) { return x + 3*vars; }
...
static void parse_convert_DIMACS_main(B& in, FILE *out, bool useInd, long int firstInds) {
    ...
    vector<bool> seenInds;  // some benchmarks include user suggestion for what the independent support should be 
    ...
    for (;;){
        ...
        if (*in == EOF) break;
            ...
            if (eagerMatch(in, "p cnf")){
                ...
                clauses = parseInt(in);
                if (printHeader) {
                    fprintf(out, "p gcnf %d %d %d\n", 4*vars, 2 * clauses + 5 * vars + 1, vars);
                    printHeader = false;
            ...
            // F(x_1, ..., x_n)
            fprintf(out, "{0} ");
            for (unsigned i=0; i<lits.size(); ++i) fprintf(out, "%d ", lits[i]);
            fprintf(out, "0\n");
            
            // F(y_1, ..., y_n)
            fprintf(out, "{0} ");
            for (unsigned i=0; i<lits.size(); ++i) fprintf(out, "%d ", y(lits[i]));
            fprintf(out, "0\n");
